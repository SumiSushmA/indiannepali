...
<div class="cust-dish-card">
    <div style="position:relative">
        <x-food-image :item="$item" :h="200" />
        @if(!empty($item['popular']))
            <div style="position:absolute;top:14px;left:14px;background:rgba(13,10,8,.7);backdrop-filter:blur(6px);border:1px solid var(--gold-700);color:var(--gold-400);font-size:11px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;padding:6px 11px;border-radius:999px">★ Popular</div>
        @endif
    </div>
    <div style="padding:22px">
        <div style="display:flex;justify-content:space-between;align-items:baseline;gap:12px">
            ...
            <span style="color:var(--gold-400);font-weight:600;font-family:var(--serif);font-size:21px">${{ $item['price'] }}</span>
        </div>
