...
<div class="adm-inbox">
    <div class="adm-card" style="display:flex;flex-direction:column;overflow:hidden;padding:0;">
        <div style="display:flex;gap:4px;padding:10px;border-bottom:1px solid var(--line);">
            ...
            <button style="flex:1;background:{{ $tab === 'All' ? 'var(--ink-600)' : 'transparent' }};border:none;border-radius:8px;padding:8px 0;cursor:pointer;color:{{ $tab === 'All' ? 'var(--cream)' : 'var(--muted)' }};font-size:13px;font-weight:600;font-family:var(--sans);">{{ $tab }}</button>
            @endforeach
        </div>
        <div style="flex:1;overflow-y:auto;">
            @foreach($contact as $m)
            <a href="{{ route('admin.inquiries.show', $m['id']) }}" style="display:block;width:100%;text-align:left;border:none;border-bottom:1px solid var(--line-soft);border-left:3px solid {{ $selected['id'] === $m['id'] ? 'var(--gold-500)' : 'transparent' }};background:{{ $selected['id'] === $m['id'] ? 'var(--ink-750)' : 'transparent' }};padding:14px 16px;cursor:pointer;color:inherit;font-family:var(--sans);text-decoration:none;">
    ...
    <div class="adm-card" style="display:flex;flex-direction:column;padding:0;">
        ...
        <div style="padding:18px;border-top:1px solid var(--line);background:var(--ink-800);">
            <form action="{{ route('admin.inquiries.reply', $selected['id']) }}" method="POST">
                @csrf
                <textarea name="reply" placeholder="Write a reply…" required style="width:100%;background:var(--ink-750);border:1px solid var(--line);border-radius:10px;padding:13px;color:var(--cream);font-size:14px;font-family:var(--sans);min-height:70px;resize:none;outline:none;"></textarea>
