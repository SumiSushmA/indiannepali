...
$pipelineValue = array_sum(array_column(array_filter($catering, fn($c) => $c['status'] !== 'New'), 'value'));
...
<div style="display:flex;justify-content:space-between;align-items:flex-end;gap:20px;flex-wrap:wrap;margin-bottom:26px;">
    ...
    <div style="display:inline-flex;background:var(--ink-800);border:1px solid var(--line);border-radius:10px;padding:3px;">
        <button style="border:none;background:var(--ink-600);color:var(--cream);padding:7px 14px;border-radius:7px;font-weight:600;font-size:13px;font-family:var(--sans);">Board</button>
...
<div class="adm-kanban">
    ...
    <div style="background:var(--ink-850);border:1px solid var(--line);border-radius:14px;padding:12px;">
        ...
        <div style="display:flex;flex-direction:column;gap:10px;">
            @foreach($cards as $c)
            <div style="background:var(--ink-700);border:1px solid var(--line);border-radius:11px;padding:14px;cursor:pointer;">
                ...
                <div style="display:flex;justify-content:space-between;align-items:center;padding-top:10px;border-top:1px solid var(--line-soft);">
                    <span style="font-family:var(--serif);font-size:17px;font-weight:600;color:var(--gold-400);">${{ number_format($c['value']) }}</span>
                    <span style="font-size:11.5px;color:var(--faint);">{{ $c['days'] === 0 ? 'today' : $c['days'] . 'd ago' }}</span>
                </div>
            </div>
            @endforeach
        </div>
...
@if(session('success'))
    <div class="adm-card" style="padding:14px 18px;margin:24px 0 16px;border-color:var(--gold-700);color:var(--gold-400)">{{ session('success') }}</div>
@endif
...
<div style="margin-top:32px;">
    ...
    <div class="adm-perm-grid">
        ...
        <div class="adm-card" style="padding:22px;">
            ...
            <form action="{{ route('admin.catering.packages.destroy', $pkg) }}" method="POST" style="margin-top:8px;" onsubmit="return confirm('Delete this package?')">
                @csrf @method('DELETE')
