...
<div id="cart-drawer" aria-hidden="true">
    ...
    <div id="cart-drawer-panel">
        <div style="padding:22px 24px;border-bottom:1px solid var(--line);display:flex;justify-content:space-between;align-items:center">
            <h2 class="cust-drawer-title">Your Order</h2>
        ...
        @else
                ...
                @foreach($cartItems as $item)
                    <div style="display:flex;gap:14px;padding:18px 0;border-bottom:1px solid var(--line-soft)">
                        <x-ph
                            ...
                            :src="\App\Support\StockImages::resolve($item['img'] ?? $item['name'] ?? '', $item['image_path'] ?? null)"
                            :h="62"
                            :w="62"
                            :r="10"
                            class="cust-cart-item-img"
                        />
                        <div style="flex:1;min-width:0">
                            <div style="display:flex;justify-content:space-between;gap:8px">
                                <div class="cust-drawer-item-name">{{ $item['name'] }}</div>
                            ...
                            <div style="display:flex;align-items:center;gap:10px;margin-top:10px">
                                <div style="display:flex;align-items:center;border:1px solid var(--line);border-radius:999px">
                                    ...
                                    <span class="cust-drawer-item-qty">{{ $item['qty'] }}</span>
                                    <form action="{{ route('cart.update', $item['id']) }}" method="POST">
                                        @csrf
                                        @method('PATCH')
            ...
            <div style="padding:24px;border-top:1px solid var(--line);background:var(--ink-800)">
                <div class="cust-drawer-line cust-text-sand" style="display:flex;justify-content:space-between;margin-bottom:8px">
                    <span>Subtotal</span><span>${{ number_format($cartSubtotal, 2) }}</span>
                </div>
                <div class="cust-drawer-line cust-text-sand" style="display:flex;justify-content:space-between;margin-bottom:14px">
                    <span>Est. tax</span><span>${{ number_format($tax, 2) }}</span>
                </div>
                <div class="cust-drawer-total" style="display:flex;justify-content:space-between;font-weight:700;margin-bottom:18px">
